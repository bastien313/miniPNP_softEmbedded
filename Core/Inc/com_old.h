...
struct transmitterController{
	volatile char buffEmit[MAX_BYTEEMIT]; // ne pas depasser 256
...
{
	if(tc->bytePending >= MAX_SIZE_FULL_BUFFER)
	{
		return 1;
	}
	else
 ...
 */
__attribute__((always_inline))
static inline void tcSend(volatile transmitterController *tc, char car)
{
	while(tcIsFull(tc));
	tc->buffEmit[tc->byteToAdd] = car;
	tc->byteToAdd++;
	tc->bytePending++;
	if(tc->byteToAdd == MAX_BYTEEMIT) tc->byteToAdd = 0; //prottction contre l'overflow
...
static inline void tcInteruptManagement(volatile transmitterController *tc)
{
	if(!tcIsVoid(tc))  // si il reste a transmetre
	{
		...
		tc->byteToSend++;
		if(tc->byteToSend == MAX_BYTEEMIT) tc->byteToSend = 0; //prottction contre l'overflow
		tc->bytePending--;
	}
	else
	{
